...
class JobApplicationEdgeCases(APITestCase):
    ...
    def test_apply_with_invalid_file_type(self):
        ...
        # The response should indicate an issue with the resume file type
        self.assertIn('resume', response.data or {})
    ...
    def test_apply_with_large_file(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        # The response should indicate an issue with the resume file size
        self.assertIn('resume', response.data or {})
    ...
    def test_apply_to_inactive_job(self):
        ...
        error_message = response.data.get('detail', response.data.get('error', str(response.data)))
        self.assertIn('not accepting applications', str(error_message).lower())
    ...
    def test_apply_twice_to_same_job(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.applicant_token}')
        data_with_job = data.copy()
        data_with_job['job'] = self.job.id  # Include job ID in the request data
        response1 = self.client.post(url, data_with_job, format='multipart')
        self.assertEqual(response1.status_code, status.HTTP_201_CREATED)
        
        # Second application (should fail)
        response2 = self.client.post(url, data_with_job, format='multipart')
...
class JobSearchEdgeCases(APITestCase):
    ...
    def test_search_with_special_characters(self):
        ...
        # Search for the Python job by title
        response = self.client.get(f"{url}?search=Python", format='json')
        
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # Should return the Python job
        self.assertEqual(len(response.data['results']), 1)
        self.assertEqual(response.data['results'][0]['id'], self.job1.id)
        
        # Test location search with special characters
        response = self.client.get(f"{url}?location=New+York%2C+NY", format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # Should return the Frontend job in New York
        self.assertEqual(len(response.data['results']), 1)
        self.assertEqual(response.data['results'][0]['id'], self.job2.id)
